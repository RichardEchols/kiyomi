"""Kiyomi "Know Me" Profile Card — A living document of who you are.
...
def generate_profile_card(config: dict) -> str:
    ...
    # ── Header ──
    sections.append(f"# 🪪 Know Me — {name}'s Profile")
    ...
    # ── Footer ──
    sections.append("\n---")
    sections.append(f"*Powered by Kiyomi — Your Personal AI Assistant*")
    sections.append(f"*{len(sections)} sections • Updated in real-time*")
...
def generate_compact_card(config: dict) -> str:
    ...
    try:
        ...
        if people:
            lines.append("👥 **My People**")
            for p in people:
                rel = p.get("relationship", "")
                bday = f" 🎂 {p.get('birthday', '')}" if p.get("birthday") else ""
                lines.append(f"  • {p['name']} ({rel}){bday}")
            lines.append("")
    ...
    try:
        ...
        if data.get("medications") or data.get("vitals") or data.get("symptoms"):
            ...
            for v in data.get("vitals", [])[-2:]:
                lines.append(f"  • {v.get('type', '')}: {v.get('value', '')}")
            lines.append("")
    except (ImportError, Exception):
        pass
    ...
    try:
        ...
        if tasks:
            ...
            for t in tasks[:3]:
                lines.append(f"  • {t.get('text', t.get('title', ''))[:50]}")
            lines.append("")
    except (ImportError, Exception):
        pass
    ...
    lines.append("─" * 25)
    lines.append("_Powered by Kiyomi_ 💛")
...
def generate_doctor_card(config: dict) -> str:
    ...
    try:
        ...
        if vitals:
            ...
            for v in vitals[-5:]:
                lines.append(f"  • {v.get('date', '')}: {v.get('type', '')} — {v.get('value', '')}")
            lines.append("")
        ...
        if symptoms:
            ...
            for s in symptoms[-5:]:
                lines.append(f"  • {s.get('date', '')}: {s.get('description', '')}")
            lines.append("")
    except (ImportError, Exception):
        pass
...
def _get_people_section() -> str:
    ...
    try:
        ...
        for p in people[:10]:
            ...
            line = f"{emoji} **{p['name']}** — {rel.capitalize()}"
            if p.get("birthday"):
                line += f" (🎂 {p['birthday']})"
            lines.append(line)
...
def _get_habits_section() -> str:
    ...
    try:
        ...
        lines = ["## 🏋️ Habits & Streaks\n"]
        
        for habit in hs.data["habits"]:
            if not habit.get("active", True):
                continue
            streak = hs._get_streak(habit["key"])
            fire = "🔥" * min(streak // 7, 5) if streak >= 7 else ""
            lines.append(f"• **{habit['name']}** — {streak}-day streak {fire}")
