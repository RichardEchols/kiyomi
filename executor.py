"""
Kiyomi Executor - Claude Code SDK bridge with persistent memory

Enhanced with:
- SDK bridge for multi-turn Claude Code sessions (terminal-quality)
- Project awareness
- Milestone tracking
- Session state
- Smart deployment
...
SDK_BRIDGE_PORT = 3456
SDK_BRIDGE_URL = f"http://127.0.0.1:{SDK_BRIDGE_PORT}"
SDK_BRIDGE_TIMEOUT = 600  # 10 minutes for complex tasks via SDK
USE_SDK_BRIDGE = True  # Set to False to fall back to subprocess
...
def _sdk_bridge_available() -> bool:
    """Check if the SDK bridge is running."""
    try:
        req = Request(f"{SDK_BRIDGE_URL}/health")
...
async def _call_sdk_bridge(prompt: str, cwd: str, user_id: str = "richard",
                           system_prompt: str = "", max_turns: int = 50,
                           new_session: bool = False) -> dict:
    """Call the SDK bridge for a multi-turn Claude Code session."""
    payload = json.dumps({
        "prompt": prompt,
        "userId": user_id,
        "cwd": cwd,
    ...
    try:
        result = await asyncio.wait_for(
            loop.run_in_executor(None, lambda: _do_sdk_request(req)),
            timeout=SDK_BRIDGE_TIMEOUT,
        )
        return result
    except asyncio.TimeoutError:
        return {"success": False, "error": "SDK bridge request timed out"}
    except Exception as e:
        return {"success": False, "error": str(e)}


def _do_sdk_request(req: Request) -> dict:
    """Synchronous HTTP request to SDK bridge (run in executor)."""
...
"""
    ...
    try:
        # Try SDK bridge first (multi-turn, terminal-quality sessions)
        use_sdk = USE_SDK_BRIDGE and _sdk_bridge_available()

        if use_sdk:
            logger.info("Using SDK bridge for multi-turn session")
            if progress_callback:
                await progress_callback("🧠 Working on it...")

            sdk_result = await _call_sdk_bridge(
                prompt=full_prompt,
                cwd=working_dir,
                user_id="richard",
                max_turns=50,
            ...
            if sdk_result.get("success"):
                output = sdk_result.get("result", "")
                sdk_cost = sdk_result.get("cost", 0)
                sdk_turns = sdk_result.get("turns", 0)
                sdk_tokens = sdk_result.get("tokens", {})
                sdk_progress = sdk_result.get("progress", [])

                # Send progress steps to Telegram so Richard can see what happened
                if progress_callback and sdk_progress:
                    # Pick key steps (not every single one — avoid spam)
                    steps = sdk_progress[:8]  # Max 8 progress messages
                    if len(steps) > 3:
                        # For lots of steps, summarize into a single message
                        step_lines = "\n".join(f"  → {s}" for s in steps)
                        await progress_callback(f"⚡ Completed {sdk_turns} turns:\n{step_lines}")
                    else:
                        for step in steps:
                            await progress_callback(f"  → {step}")

                logger.info(f"SDK bridge: {sdk_turns} turns, ${sdk_cost:.4f}")

                # Log cost with real data from SDK
                cost = log_api_call(
                    model="claude-opus-4-6",
                    input_tokens=sdk_tokens.get("input", 0),
                    output_tokens=sdk_tokens.get("output", 0),
                ...
                # SDK bridge failed, fall through to subprocess
                error_msg = sdk_result.get("error", "Unknown SDK error")
                logger.warning(f"SDK bridge failed: {error_msg}, falling back to subprocess")
                use_sdk = False

        if not use_sdk:
            # Fallback: single-shot subprocess (original behavior)
            logger.info("Using subprocess fallback (single-shot)")
            process = await asyncio.create_subprocess_exec(
                CLAUDE_CLI_PATH,
                "-p", full_prompt,
                "--dangerously-skip-permissions",
